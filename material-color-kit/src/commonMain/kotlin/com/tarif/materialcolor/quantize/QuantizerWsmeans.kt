...
internal object QuantizerWsmeans {
    ...
    fun quantize(
        ...
        val pointProvider: com.tarif.materialcolor.quantize.PointProvider =
            com.tarif.materialcolor.quantize.PointProviderLab()
        var pointCount = 0
        ...
        val distanceToIndexMatrix: Array<Array<com.tarif.materialcolor.quantize.QuantizerWsmeans.Distance?>> = Array(clusterCount) {
            Array(clusterCount) { com.tarif.materialcolor.quantize.QuantizerWsmeans.Distance() }
        }
        for (i in 0 until clusterCount) {
            distanceToIndexMatrix[i] = arrayOfNulls(clusterCount)
            for (j in 0 until clusterCount) {
                distanceToIndexMatrix[i][j] =
                    com.tarif.materialcolor.quantize.QuantizerWsmeans.Distance()
            }
        }
        val pixelCountSums = IntArray(clusterCount)
        for (iteration in 0 until com.tarif.materialcolor.quantize.QuantizerWsmeans.MAX_ITERATIONS) {
            for (i in 0 until clusterCount) {
            ...
            for (i in 0 until pointCount) {
                ...
                if (newClusterIndex != -1) {
                    ...
                    if (distanceChange > com.tarif.materialcolor.quantize.QuantizerWsmeans.MIN_MOVEMENT_DISTANCE) {
                        pointsMoved++
    ...
    private class Distance : Comparable<com.tarif.materialcolor.quantize.QuantizerWsmeans.Distance> {

        var index: Int = -1
        var distance: Double = -1.0

        override operator fun compareTo(other: com.tarif.materialcolor.quantize.QuantizerWsmeans.Distance): Int {
