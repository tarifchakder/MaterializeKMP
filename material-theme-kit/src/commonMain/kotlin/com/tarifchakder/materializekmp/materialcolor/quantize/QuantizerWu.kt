...
import com.tarifchakder.materializekmp.materialcolor.utils.ColorUtils.blueFromArgb
import com.tarifchakder.materializekmp.materialcolor.utils.ColorUtils.greenFromArgb
import com.tarifchakder.materializekmp.materialcolor.utils.ColorUtils.redFromArgb
...
internal class QuantizerWu : Quantizer {
    ...
    private fun cut(one: Box?, two: Box?): Boolean {
        ...
        val maxRResult = maximize(one,
            Direction.RED, one!!.r0 + 1, one.r1, wholeR, wholeG, wholeB, wholeW)
        val maxGResult = maximize(one,
            Direction.GREEN, one.g0 + 1, one.g1, wholeR, wholeG, wholeB, wholeW)
        val maxBResult = maximize(one,
            Direction.BLUE, one.b0 + 1, one.b1, wholeR, wholeG, wholeB, wholeW)
